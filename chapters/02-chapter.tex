...
O presente Trabalho de Conclusão de Curso propõe-se a realizar uma análise comparativa entre os protocolos de comunicação REST e gRPC em arquiteturas de microsserviços, utilizando métricas de observabilidade para avaliar aspectos de desempenho como latência, \textit{throughput} e uso de recursos. A motivação central reside na crescente adoção de microsserviços em ambientes corporativos e acadêmicos, onde a escolha do protocolo de comunicação pode impactar diretamente a eficiência, escalabilidade e manutenção dos sistemas.
...
O ambiente experimental será configurado em um \textit{cluster} de orquestração de contêineres, possibilitando a simulação de cenários reais de operação e a execução de testes de carga controlados. Serão definidos diferentes perfis de carga, variando o número de requisições e o volume de dados trafegados, com o objetivo de avaliar o comportamento dos protocolos sob distintas condições de uso. As métricas de latência, \textit{throughput} e consumo de recursos computacionais serão coletadas de forma sistemática durante os experimentos.
